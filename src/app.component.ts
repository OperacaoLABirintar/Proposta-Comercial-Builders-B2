    ...
    <div class="bg-white rounded-xl shadow-lg p-4 md:p-8">
      <div class="grid md:grid-cols-5 gap-12">
        ...
        <div class="md:col-span-2 space-y-8">
          <div>
            <h3 class="text-2xl font-bold mb-4 text-zinc-800">Estrutura Semanal</h3>
            <div class="p-5 rounded-lg bg-[#ffe9c9]">
              <p class="text-zinc-700">Os alunos participam de projetos fixos, com experiências de uma hora, oferecidas uma vez por semana, possibilitando algum tipo de integração.</p>
            </div>
          </div>
          <div>
            <h3 class="text-2xl font-bold mb-4 text-zinc-800">Eixos Pedagógicos</h3>
            <div class="p-5 rounded-lg bg-[#ffe9c9]">
              <p class="text-zinc-700 mb-2">Essas experiências são voltadas para alunos de 10 a 14 anos e contemplam os eixos:</p>
              <ul class="text-zinc-700 space-y-1 font-medium">
                <li>Arte, Cultura e Expressão</li>
                <li>Corpo e Movimento</li>
                <li>Fazeres Manuais</li>
                <li>Jogo, Estratégia e Lógica</li>
                <li>Tecnologias Criativas e Sustentáveis</li>
                <li>Cidade e Cultura Viva</li>
              </ul>
            </div>
          </div>
        </div>
        <!-- Coluna do Menu de Experiências -->
        <div class="md:col-span-3">
          <h3 class="text-2xl font-bold mb-4 text-zinc-800">Experiências B2 - 2026</h3>
          <div class="flex flex-col space-y-3">
            @for(exp of experiencias; track exp.title) {
              <button (click)="selectExperiencia(exp)"
                class="w-full text-left font-bold p-4 rounded-lg transition-all duration-300 text-lg"
                [class]="selectedExperiencia() === exp ? exp.color + ' text-zinc-900 shadow-lg transform scale-105' : 'bg-zinc-100 hover:bg-zinc-200 text-zinc-700'">
                {{ exp.title }}
              </button>
            }
          </div>
     ...
     <p class="text-center mt-8 text-zinc-600 italic">A descrição completa de cada experiência encontra-se no PDF em anexo.</p>
  </div>
</section>
  `,
  changeDetection: ChangeDetectionStrategy.OnPush,
...
class ProgramComponent {
  experiencias: ExperienceMenuItem[] = [
    { title: 'Marcenaria Criativa', color: 'bg-[#e6cbe4]' },
    { title: 'Robótica Sustentável', color: 'bg-[#e6cbe4]' },
    { title: 'Improvisação', color: 'bg-[#aec5e7]' },
    { title: 'Xadrez para a Vida', color: 'bg-[#aec5e7]' },
    { title: 'Circuito Acrobático Circense', color: 'bg-[#b2dcd5]' },
    { title: 'Parkour Brasil', color: 'bg-[#b2dcd5]' },
    { title: 'Cozinhas & Infâncias: Cultura alimentar e Sustentabilidade', color: 'bg-[#ffe9c9]' }
  ];
  selectedExperiencia = signal<ExperienceMenuItem>(this.experiencias[0]);

  selectExperiencia(exp: ExperienceMenuItem) {
    this.selectedExperiencia.set(exp);
  }
}
